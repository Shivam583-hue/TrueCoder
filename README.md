...
- **Refusals you can act on** - when a provider turns a request down you get a sentence, not its wire format: what happened, the provider's own explanation, and the next step. A rejected key opens a prompt for another one; a rejected OAuth token points at `/login`; running out of credit suggests neither, because a new credential does not buy anything. A failure that does not classify gets no invented advice.
...
| -------------------------- | ----------------------------------------------: | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Physical source lines      |                      **39,319** across 170 files | Python under `src/truecoder`, excluding tests, the sandbox image, and generated packaging metadata.                                                           |
| Execution subsystem share  |                    **19,435 lines**, 50% of src | The execution control plane, audit store, and platform backends. Tools are 4,188 lines, the TUI is 4,553, the agent is 2,889, providers are 1,698, MCP is 957, LSP is 951, checkpoints are 735, the client is 619, web is 655, sessions are 518, JSON-RPC is 424, memory is 407, hooks are 402, mutation is 281, evaluation is 245, the CLI is 184, and planning is 146. |
| Test lines                 |                      **41,749** across 221 files | The complete Python test tree, including fakes and child-process helpers; a test-to-source ratio of roughly 1.06 to 1.                                       |
| Automated scenarios        |                      **2,373**, locally clean   | 2,056 unit, 226 integration, 41 contract, 28 end-to-end, and 22 sandbox scenarios. On Linux, 2,363 pass and 13 Windows-only scenarios skip.                     |
| Unit suite                 |                  **2,056 passing in 24.1 seconds** | Mostly pure logic with injected boundaries; platform-specific filesystem and native-boundary cases are explicitly scoped to their supported hosts.           |
...
with its models in it. Signing in to a provider does not move the session to it;
that happens when you choose one of its models.
...
Current inventory: **2,309 scenarios**.
In the local Linux verification, 2,296 pass and 13 Windows-only scenarios skip;
